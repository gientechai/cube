...
pub trait Request {
    ...
    fn headers(&self) -> Result<HeaderMap, S3Error> {
        ...
        match self.command() {
            ...
            // Body-less commands: signing a content-length header that the
            // HTTP client then omits (hyper strips Content-Length for empty
            // bodies) is rejected by strict SigV4 implementations such as
            // Garage with 400 "signed header `content-length` is not present".
