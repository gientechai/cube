...
- Kingbase Oracle Tesseract coverage includes rolling windows without `dateRange`; the Kingbase Oracle Target renders generated time series through recursive CTE templates.
- Kingbase Oracle Tesseract coverage includes bound filters on time-shift measures; a throwaway prototype verified the previously skipped path now succeeds through the public Cube API.
- Driver-suite failures that report `DB_CAST[type] is undefined` are test-fixture target mapping failures, not Driver Transport or SQL Dialect failures. The driver-suite fixture data currently knows `postgres` but not the Kingbase Target names, so `kingbase-pg` should reuse the Postgres cast/date fixture mapping and `kingbase-oracle` needs an Oracle-compatible fixture mapping.
- Kingbase Oracle driver-suite failures for `countDistinctApprox` are SQL Dialect capability failures or unsupported-capability expectations, not fixture mapping failures. The Kingbase Oracle Target inherits Oracle semantics unless a Kingbase-specific override is validated, and approximate distinct count support has not been validated for that target.
