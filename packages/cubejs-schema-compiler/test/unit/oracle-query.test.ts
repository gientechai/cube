...
describe('OracleQuery', () => {
  ...
  it('Rewrites owned cube qualified references for Oracle quote styles', async () => {
    await compiler.compile();
    ...
    const otherCube = query.withCubeAliasPrefix(
      'main',
      () => query.autoPrefixWithCubeName('visitors', '"orders".amount'),
    );

    expect(quoted).toEqual('"main__visitors".amount');
    expect(unquoted).toEqual('"main__visitors".amount');
    expect(otherCube).toEqual('"orders".amount');
  });
});

describe('OracleQuery period_average SQL dialect', () => {
  ...
  it('calendar/day month bucket uses LAST_DAY and avoids Postgres ::date casts', async () => {
  ...
  it('range month divisor uses MONTHS_BETWEEN instead of AGE', async () => {
    ...
    expect(divisor).not.toMatch(/::\w+/);
    expect(divisor).toMatch(/DATE '2025-/);
  });
  ...
  it('cumulative day/month uses TRUNC instead of DATE_TRUNC', async () => {
    ...
    expect(numerator).not.toMatch(/::\w+/);
